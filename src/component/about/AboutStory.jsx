        ...
        <section className='about_story_section padding-inline xl:mx-[100px] sm:mx-[30px] md:mt-0 mt-[40px] '>
            ...
            <div className='story_card_section grid md:grid-cols-2 grid-cols-1 lg:gap-8 gap-5'>
                {storyContent.map((element, index) => {
                    return (
                        <div className="story_card flex lg:flex-row flex-col lg:items-center lg:gap-x-6 bg-light-white py-5 md:ps-5 ps-3 lg:pe-10 md:pe-8 pe-5 rounded-[25px]" key={index} data-aos='fade-up' data-aos-duration='1800' data-aos-anchor-placement="center-bottom"  >
                            <div className="story_card_icon mb-lg-0 mb-4">
                                <p className='w-[60px] h-[70px] rounded-lg bg-sky-900 text-white text-[30px] flex items-center justify-center'>{element.icon}</p>
                            </div>
                            <div className="story_card_content">
                                <h5 className='text-black  font-bold xl:text-[25px] text-[20px] mb-1 font-primary  tracking-wide'>{element.title}</h5>
                                <p className=' text-gray text-justify text-justify-last font-secondary'>{element.content}</p>
                            </div>
                        </div>
                    )
                })}
