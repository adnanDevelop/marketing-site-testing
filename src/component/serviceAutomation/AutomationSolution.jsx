      ...
      <div className='grid lg:grid-cols-12 gap-x-[10px] lg:mx-[70px]'>
        <div className='solution_tabs lg:col-span-3 grid md:grid-cols-3 sm:grid-cols-2 lg:grid-cols-1 gap-3'>
          {automationTabContent.map((element, index) => {
            return (
              <div key={index} className="tab mb-1 px-[10px] py-[20px] text-center flex items-center justify-center flex-col rounded-[15px] border border-[#CBCBCB] bg-light-white w-full lg:max-w-[15rem] mx-auto">
                <div data-aos='zoom-in' data-aos-duration='1800'>
                  <img src={element.icon} className='w-[2rem] mx-auto mb-2' alt="" />
                  <p className='text-black font-primary tracking-[0.5px] font-extrabold text-[17px] mt-4 leading-none'>{element.title}</p>
                </div>
              </div>
            )
          })}
